    ...
    <div v-show="buttonKey==AP_STATUS.WAIT">
      <avue-crud :option="waitOption"
        ...
        <template slot="menuLeft">
          ...
          <el-button type="primary" size="mini" @click="downloadContract">打印合同</el-button>
          <el-button type="primary" size="mini" @click="checkData">上传合同</el-button>
          <el-upload
                  class="upload-demo"
                  ref="upload"
                  :action=uploadUrl
            ...
            <el-button type="primary" size="mini" ref="telUpload" style="display: none;">上传合同</el-button>
          </el-upload>
        ...
        <template slot-scope="scope" slot="vmiContract">
          <el-button type="text"
                     v-if="scope.row.vmiContract !=''"
                     size="mini"
                     @click="viewImages(scope.row.vmiContract)">查看附件
          </el-button>
        </template>
...
<script>
    ...
    import {getVmiPageList,uploadContract}                from "@/api/ap/aprcv";
    import {getVmiApList, getVmiStatistics, AP_STATUS, saveAps} from "@/api/ap/ap";
    ...
    import {accept, getDetail, PO_STATUS, CON_STATUS, PO_STATUS_PAIR, updatecontract, cancel} from "@/api/po/po";
        ...
        methods   :{
          ...
          viewImages(url){
            ...
            downloadContract(po){
              ...
              let vmiCode = selData[0].poCode.split(" ")[0]
              for (let rowData of selData) {
                if (rowData.poCode.split(" ")[0] != vmiCode) {
                  return this.$message.warning("请选择相同订单编号的数据！");
                }
              }
              let rcvIds = '';
              for (let rowData of selData) {
                rcvIds = rcvIds + ',' + rowData.reqId
              }
              window.open("/print_0716_vmi.html?rcvIds=" + rcvIds.substr(1,rcvIds.length))
            },
            //上传合同
            checkData(){
              if (this.selectionList.length === 0) {
              ...
              for (let rowData of selData) {
                ...
                if (rowData.vmiStatus == '30') { // 审核通过的合同就不能再更改了
                  return this.$message.warning(rowData.poCode + ': 该订单编号已经不能再上传合同了，请确认！');
                }

                if (rowData.vmiContract != '') {
                  conIsExisted = true;
                }
              }
              if(conIsExisted) {
                this.$confirm('合同已经存在，是否要替换?', '提示', {
            ...
            handleSuccess(res, file, fileList){
              ...
              this.fileList = []
              let uploadUrlList = this.urlData.join('|');
              if(res.code === 200){
                let selData = JSON.parse(JSON.stringify(this.selectionList))
                let rcvIds = '';
                for (let rowData of selData) {
                ...
                rcvIds = rcvIds.substr(1,rcvIds.length);
                uploadContract({'rcvIds':rcvIds,'uploadUrlList':uploadUrlList,'vmiStatus':CON_STATUS.AUDIT}).then(() => {
                  this.$message({
                    message:'上传成功！',
                    type   :'success'
                  });
            ...
            handleExcel(){
              if (this.selectionList.length>0){
                let req = {
                  ...
                  selectionIds: this.selectionList.map(row => row.reqId,).join(","),
                };
                window.open('/api/blade-ap/aprcv/vmiExport?' + objToUrlParam(req));
              }else {
                let req = {
                  "Blade-Auth": this.$store.getters.token,
                  ...this.query
            ...
            onLoad(page, params = {}){
                ...
                getVmiPageList(page.currentPage, page.pageSize, req).then(res => {
                    this.loading    = false;
            ...
            reconciliation(){
                ...
                for(var i = 0; i < this.selectionList.length; i++){
                    ...
                    if(this.selectionList[i].vmiStatus!='30') {
                      return this.$message.error(this.selectionList[i].poCode + ": 合同还未审核通过，暂不能对账!");
                    }
            ...
            onLoadReconciled(page, params = {}){
              ...
              getVmiApList(page.currentPage, page.pageSize, req).then(res => {
                    const data                = res.data.data;
            ...
            detail(row){
                ...
                this.reconDetailSelectData.vmiType = 1
