...
<script>
    import {getList, getDetail, add, update, remove, getPoItem} from "@/api/po/po";
        ...
        methods: {
            ...
            handleExcel() {
              if (this.selectionList.length>0){
                //若有选中行的情况,按照选中进行打印
                // this.$refs.crud.rowExcel();  //根据页面列进行导出功能
                let req ={
                  "Blade-Auth": this.$store.getters.token,
                  selectionIds: this.selectionList.map(row => row.id,).join(","),
                }
                window.open('/api/blade-po/poitem/exportByPo?' + objToUrlParam(req))
              }else{
                let req = {
                  "Blade-Auth": this.$store.getters.token,
                  ...this.query
