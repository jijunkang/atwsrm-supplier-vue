<template>
  <el-dialog title="订单详情"
    ...
    <container>
      <div id="avue-detail">
        <avue-detail :option="po_detail" v-model="form">
          <template slot-scope="scope" slot="contractForm">
            <div id="viewBox">
               <span v-for="(list,index) in fileListUrl" id="viewFile">
              <el-button class="pic" type="text" icon="el-icon-picture-outline" size="medium" :disabled="scope.row.contract ==''"
                         @click="viewImages(list.url)"></el-button>
              <el-button type="text" id="view" class="big" icon="el-icon-view" size="medium" :disabled="scope.row.contract ==''" @click="viewImages(list.url)">
              </el-button>
                    <el-button
                        v-if="form.contractStatus==CON_STATUS.INIT || form.contractStatus==CON_STATUS.AUDIT || form.contractStatus==CON_STATUS.FAIL"
                        type="text" class="viewImg" icon="el-icon-circle-close" size="medium" @click="handleRemove(index)"></el-button>
            </span>
            </div>
      ...
      <avue-crud :data="poData.records"
        ...
        <template slot="menuLeft">
          <div style="margin-bottom:15px">
            ...
            <span v-if="form.contractStatus != CON_STATUS.SUCCESS">
            <el-button type="primary" size="mini" @click="downloadContract(form)">打印合同</el-button>
              <el-upload
                  class="upload-demo"
                  ref="upload"
                  :action=uploadUrl
        ...
        <!--        状态-->
        <template slot="status" slot-scope="scope">{{form.status|statusFmt(scope.row.u9StatusCode,scope.row.arvGoodsNum)}}</template>
      </avue-crud>
      <!--物料编码-->
